...
因此它的实验数值和 StreamingLLM 相同。把它保留下来是为了对应 PPT 里的参考方法，并在报告中说明这种方法族的共同点。
...
它比较激进，因为它不强制保留完整最近窗口，所以可能会丢掉一些局部连续性信息。
...
| lm_infinite | 31.43 | 260 | 227.09 |
| h2o | 31.39 | 288 | 247.60 |
| scissorhands | 31.43 | 288 | 247.60 |
| tova | 31.37 | 288 | 247.60 |
...
| lm_infinite | 36.19 | 260 | 227.09 |
| h2o | 35.51 | 288 | 247.60 |
| scissorhands | 36.44 | 288 | 247.60 |
| tova | 37.36 | 288 | 247.60 |
...
不过也可以看到，`tova` 在 Wikitext-2 上反而较差，PPL 为 37.36。这可能是因为本实验的 TOVA-lite 不强制保留完整最近窗口，只保留当前 token 和高注意力历史 token，因此会丢掉一些局部连续上下文。
...
从速度看，本实验的实现是为了清楚和可复现，而不是为了极致优化。它使用 Python 循环逐 token 推理，所以绝对速度不能代表 vLLM、TensorRT-LLM 这类高性能推理框架。不过在同一份代码里，压缩方法整体表现出更低的缓存规模和更好的吞吐率。
...
本实验复现并比较了多种训练无关的 KV Cache 压缩方法。实验说明，完整 KV Cache 效果最好但缓存最大；纯滑动窗口最简单但质量损失明显；StreamingLLM/LM-Infinite 通过保留初始 token 和局部窗口能显著改善效果；H2O-lite、SnapKV-lite、PyramidKV-lite 等注意力选择方法可以进一步提升困惑度。
